...
Mat Dip2::spatialConvolution(Mat& src, Mat& kernel){
   // TO DO !!
	...
	mat.col(1).row(2) = 8;
	mat.col(2).row(2) = 9;
	cout << "kernel before flip" << endl;
	//cout << tmp << endl;
	//cout << mat << endl;
	//cout << mat.col(0).row(0) << "-" << mat.col(1).row(0) << "-" << mat.col(2).row(0) << endl;
	//cout << mat.col(0).row(1) << "-" << mat.col(1).row(1) << "-" << mat.col(2).row(1) << endl;
	//cout << mat.col(0).row(2) << "-" << mat.col(1).row(2) << "-" << mat.col(2).row(2) << endl;
	// "-" << kernel.at<uchar>(Point(1, 0)) << "-" << kernel.at<uchar>(Point(2, 0)) << endl;
	//cout << kernel.at<uchar>(Point(0, 1)) << "-" << kernel.at<uchar>(Point(1, 1)) << "-" << kernel.at<uchar>(Point(2, 1)) << endl;
	//cout << kernel.at<uchar>(Point(0, 2)) << "-" << kernel.at<uchar>(Point(1, 2)) << "-" << kernel.at<uchar>(Point(2, 2)) << endl;

	if (src.empty()) { cout << "src empty" << endl; return src.clone(); }
	if (kernel.empty()) { cout << "kernel empty" << endl; return src.clone(); }

   //1. flip kernel

	for (int x = 0; x < (mat.cols - 1) / 2; x++) {
		for (int y = 0; y < (mat.rows - 1) / 2; y++) {
			cout << mat.col(mat.cols - 1 - x).row(mat.rows - 1 - y) << endl;
			mat.col(x).row(y) = mat.col(mat.cols -1- x).row(mat.rows -1- y);
			
		}
	}
	//tmp.copyTo(mat);
	cout << "kernel after flip" << endl;
	cout << tmp << endl;
	cout << mat.col(0).row(0) << "-" << mat.col(1).row(0) << "-" << mat.col(2).row(0) << endl;
	cout << mat.col(0).row(1) << "-" << mat.col(1).row(1) << "-" << mat.col(2).row(1) << endl;
	cout << mat.col(0).row(2) << "-" << mat.col(1).row(2) << "-" << mat.col(2).row(2) << endl;
	cout << tmp.col(0).row(0) << "-" << tmp.col(1).row(0) << "-" << tmp.col(2).row(0) << endl;
	cout << tmp.col(0).row(1) << "-" << tmp.col(1).row(1) << "-" << tmp.col(2).row(1) << endl;
	cout << tmp.col(0).row(2) << "-" << tmp.col(1).row(2) << "-" << tmp.col(2).row(2) << endl;
	//cout << kernel.at<uchar>(Point(0,0))<<"-"<< kernel.at<uchar>(Point(1, 0)) << "-" << kernel.at<uchar>(Point(2, 0)) << endl;
	//cout << kernel.at<uchar>(Point(0, 1)) << "-" << kernel.at<uchar>(Point(1, 1)) << "-" << kernel.at<uchar>(Point(2, 1)) << endl;
	//cout << kernel.at<uchar>(Point(0, 2)) << "-" << kernel.at<uchar>(Point(1, 2)) << "-" << kernel.at<uchar>(Point(2, 2)) << endl;
...
/*
src:     input image
anchor:  anchor point (in the middle)
return:  median
*/
Point2i getMedian(Mat& src, Point2i anchor, int size) {
	...
	for (int x = 0; x < size; x++) {
		...
		for (int y = 0; y < size; y++) {
			...
			integerList.push_back(src.at<uchar>(Point(srcX, srcY)));
			point2iList.push_back(Point(srcX, srcY));
		}
	}
	integerList.sort();
	...
	while(src.at<uchar>(point2iList.back()) != integerList.back()) {
...
Mat Dip2::medianFilter(Mat& src, int kSize){
   // TO DO !!
	...
	Mat outputImage = ownImage.clone();
	Point2i anchor;
   //cout << "kSize: " << kSize << endl;
   //cout << "srcSize: " << src.cols << "x" << src.rows << endl;
   for (int x = 0; x < ownImage.cols; x++) {
	   anchor.x = x;
   	for (int y = 0; y < ownImage.rows; y++) {
		anchor.y = y;
		//outputImage.at<uchar>(Point(x, y)) = ownImage.at<uchar>(getMedian(ownImage, anchor, kSize));
   	}
   }
   namedWindow("Test");
